    ...
    <section class="shop-main container d-flex pt-4 pt-xl-5">
        <div class="shop-sidebar side-sticky bg-body" id="shopFilter">
          ...
          <div class="text-right mb-4">
            <a href="{{route('public.shop.index')}}" class="menu-link py-1">Clear All</a>
          </div>
                  ...
                  <ul class="list list-inline mb-0">
                    @foreach ($categories as $category)
                          <li class="list-item d-flex justify-content-between">
                            <label for="{{$category->id}}" class="menu-link py-1">
                              <input type="checkbox" id="{{$category->id}}" name="categories" value="{{$category->id}}" class="chk-category" {{ request()->categories && in_array($category->id, explode(',', request()->categories)) ? 'checked' : '' }}>
                              {{$category->name}}
                            </label>
                ...
                <div class="search-field multi-select accordion-body px-0 pb-0">
                    <ul class="list list-inline mb-0 brand-list">
                      @foreach ($brands as $brand)
                          <li class="list-item d-flex justify-content-between">
                            <label for="{{$brand->id}}" class="menu-link py-1">
                              <input type="checkbox" id="{{$brand->id}}" name="brands" value="{{$brand->id}}" class="chk-brand" {{ request()->brands && in_array($brand->id, explode(',', request()->brands)) ? 'checked' : '' }}>
                              {{$brand->name}}
                            </label>
                            <span class="">{{$brand->products->count()}}</span>
                          </li>
                      @endforeach
                    </ul>
                </div>
            ...
            <div class="accordion-item mb-4">
              ...
              <div id="accordion-filter-price" class="accordion-collapse collapse show border-0" aria-labelledby="accordion-heading-price" data-bs-parent="#price-filters">
                ...
                <input class="price-range-slider" type="text" name="price_range" 
                    value="{{ request('price_range') ?? '0,450' }}" 
                    data-slider-min="0" 
                    data-slider-max="1000" 
                    data-slider-step="5" 
                    data-slider-value="[{{ request('price_range') ?? '0,450' }}]" 
                    data-currency="$" />

                <div class="price-range__info d-flex align-items-center mt-2">
                    <div class="me-auto">
                        <span class="text-secondary">Min Price: </span>
                        <span class="price-range__min">${{ explode(',', request('price_range'))[0] ?? 250 }}</span>
                    </div>
                    <div>
                        <span class="text-secondary">Max Price: </span>
                        <span class="price-range__max">${{ explode(',', request('price_range'))[1] ?? 450 }}</span>
                    </div>
                </div>
            </div>
            </div>
        ...
        <div class="shop-list flex-grow-1">
                ...
                <div class="slide-split h-100 d-block d-md-flex overflow-hidden">
                  ...
                  <div class="slide-split_media position-relative">
                    <div class="slideshow-bg" style="background-color: #f5e6e0;">
                      <img loading="lazy" src="{{asset('frontend/images/shop/shop_banner3.jpg')}}" width="630" height="450"
          ...
          <div class="d-flex justify-content-between mb-4 pb-md-2">
            @include('public.pages.partials.breadcrumb', ['header' => 'Shop'])
  
            <div class="shop-acs d-flex align-items-center justify-content-between justify-content-md-end flex-grow-1">
              <select id="shortedBy" class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0" aria-label="Sort Items"
                name="shortBy">
                <option value="">Default Sorting</option>
                <option value="1" {{ request('shortBy') == '1' ? 'selected' : '' }}>Alphabetically, A-Z</option>
                <option value="2" {{ request('shortBy') == '2' ? 'selected' : '' }}>Alphabetically, Z-A</option>
                <option value="3" {{ request('shortBy') == '3' ? 'selected' : '' }}>Price, low to high</option>
                <option value="4" {{ request('shortBy') == '4' ? 'selected' : '' }}>Price, high to low</option>
                <option value="5" {{ request('shortBy') == '5' ? 'selected' : '' }}>Date, old to new</option>
                <option value="6" {{ request('shortBy') == '6' ? 'selected' : '' }}>Date, new to old</option>
              </select>
          ...
          <div class="products-grid row row-cols-2 row-cols-md-3" id="products-grid">
            @foreach ($products as $product)
                @include('public.pages.partials.product')
            @endforeach
          </div>
                ...
                <ul class="pagination mb-0">
                    @foreach ($products->links()->elements[0] as $page => $url)
                        <li class="page-item {{ $page == $products->currentPage() ? 'active' : '' }}">
                            <a class="btn-link px-1 mx-2 {{ $page == $products->currentPage() ? 'btn-link_active' : '' }}" href="{{ $url }}">{{ $page }}</a>
                        </li>
                    @endforeach
                </ul>
          ...
          <form id="filter" action="{{route('public.shop.index')}}">
            <input type="hidden" name="categories" id="hdnCategories">
            <input type="hidden" name="brands" id="hdnBrands">
            <input type="hidden" name="shortBy" id="hdnShort">
            <input type="hidden" name="price_range" id="hdnPrice">
          </form>
...
<script>
  $(function(){
    function setCategories() {
      ...
      var sliderInstance = $(".price-range-slider").val();
      $('#hdnPrice').val(sliderInstance);
    }
    ...
    $('select[id="shortedBy"]').on('change', function(){
      setCategories();
      setBrands();
      $('#hdnShort').val(($(this).val()));
